...
func getHTTPResponseError(resp *http.Response) error {
	...
	msg := fmt.Sprintf("Napi request error: %d('%s')\nContents:\n%s\n", resp.StatusCode, resp.Status, content)
...
func (wrb *NapiRequestBuilder) BuildUrl(t RequestType, objType string, ref string, returnFields []string, queryParams *QueryParams) (urlStr string) {
	path := []string{"Napi", "v" + wrb.hostCfg.Version}
	if len(ref) > 0 {
...
func NewConnector(hostConfig HostConfig, authCfg AuthConfig, transportConfig TransportConfig,
	...
	//connector.requestBuilder = NapiRequestBuilder{WaipHostConfig: connector.hostCfg}
